...
var sortIcon = params.sortIcon;
    ...
    function jsSort(objList, byProperty, direction) {
    ...
    function addTableBodyFilter(table, list, filterValue) { 
        ...
        jsSort(list, sortOrderPropName, "Forward");
    ...
    function addTableBody(table, list, sortOrderPropName, Direction) {
        ...
        jsSort(list, sortOrderPropName, Direction);
    ...
    for (var propName in obj) {
        var headingCell = document.createElement("th", class {direction});

        // underscores in the property name will be replaced by space in the column headings.
        headingText = propName.replace("_", " ");
        // give each Header tag a string attribute indicating sort direction
        headingCell.setAttribute("direction", "Reverse");
        ...
        if (propName[0] !== "_") {
            ...
            headingCell.onclick = function () {
                ...
                //Checks for header tag directionality, then switches 
                //direction value 
                if (headingCell.getAttribute("direction") === "Forward"){
                    console.log("WILL SORT ON " + this.sortPropName);
                    addTableBody(newTable, objList, this.sortPropName, "Forward"); 
                    headingCell.setAttribute("direction", "Reverse");
                    console.log("DEBUG FORWARD");
                }
                else if (headingCell.getAttribute("direction") === "Reverse"){
                    console.log("WILL SORT ON " + this.sortPropName);
                    addTableBody(newTable, objList, this.sortPropName, "Reverse");  
                    headingCell.setAttribute("direction", "Forward");
    ...
    searchInput.onkeyup = function () {
        ...
        addTableBodyFilter(newTable, objList, searchInput.value);
    };
    addTableBody(newTable, objList, sortOrderPropName, "Forward");
