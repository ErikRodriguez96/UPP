...
// Properties that are "global" to this IIFE: 
    ...
    function createValidationArea() {
        ...
        makeInputRow("photoId", "Photo Id", validateTable);
        validateObjList["photoId"].inputBox.setAttribute("disabled", true);

        makeInputRow("title", "Title", validateTable);
        makeInputRow("location", "Location", validateTable);
        makeInputRow("dateTaken", "Date Taken", validateTable);
        makeInputRow("imageUrl", "Image URL", validateTable);
        makeInputRow("blurb", "Blurb", validateTable);

        // initially, the user role is an input box like the other fields, but it will get 
        // changed to a select tag...
        makeInputRow("webUserId", "User Email", validateTable);
    ...
    function getDataFromUI() {     // a private function within the IIFE 
        ...
        var photoInputObj = {
            ...
            "photoId": validateObjList["photoId"].inputBox.value,
            "title": validateObjList["title"].inputBox.value,
            "location": validateObjList["location"].inputBox.value,
            "dateTaken": validateObjList["dateTaken"].inputBox.value,
            "imageUrl": validateObjList["imageUrl"].inputBox.value,
            "blurb": validateObjList["blurb"].inputBox.value,
        ...
        console.log("getDataFromUI - photoInputObj on next line");
        console.log(photoInputObj);
        ...
        return encodeURIComponent(JSON.stringify(photoInputObj));
    }
    ...
    function writeErrorObjToUI(jsonObj) {
        ...
        console.log(jsonObj);
        validateObjList["title"].errorTd.innerHTML = jsonObj.title;
        validateObjList["location"].errorTd.innerHTML = jsonObj.location;
        validateObjList["dateTaken"].errorTd.innerHTML = jsonObj.dateTaken;
        validateObjList["imageUrl"].errorTd.innerHTML = jsonObj.imageUrl;
        validateObjList["blurb"].errorTd.innerHTML = jsonObj.blurb;
    ...
    function insertSave() {
        ...
        ajax("webAPIs/insertPhotoAPI.jsp?jsonData=" + myData, processInsert, recordError);
        function processInsert(obj) {

            console.log("photoMods.insert/insertSave/processInsert error msg obj (see next line)");
            console.log(obj);
    ...
    photoMods.insert = function () {
        ...
        ajax("webAPIs/getEmailsAPI.jsp", processEmails, insertDiv);
    ...
    photoMods.update = function (photoId) {

        console.log("photoMods.update called with photoId " + photoId);
        ...
        ajax("webAPIs/getPhotoByIdAPI.jsp?photoId=" + photoId, gotRecordById, updateDiv);

        function gotRecordById(photoObj) { // obj is what got JSON.parsed from Web API's output

            console.log("gotRecordById, photoObj is next");
            ...
            validateObjList["photoId"].inputBox.value = photoObj.photoId;
            validateObjList["title"].inputBox.value = photoObj.title;
            validateObjList["location"].inputBox.value = photoObj.location;
            validateObjList["dateTaken"].inputBox.value = photoObj.dateTaken;
            validateObjList["imageUrl"].inputBox.value = photoObj.imageUrl;
            validateObjList["blurb"].inputBox.value = photoObj.blurb;
            validateObjList["webUserId"].inputBox.value = photoObj.webUserId;

            // replace role id inputBox with select tag populated from the roles in the database.
            // NOTE: since roles do not change that much, it would be OK to not be so careful 
            // to get the latest roles from the db to populate the role pick list. I am showing this 
            // to you so that you WOULD KNOW how to get the latest pick list from the DB.
            ajax("webAPIs/getEmailsAPI.jsp", processEmails, updateDiv);
            ...
            function processEmails(obj) {
                ...
                if (obj.dbError.length > 0) {
                    ...
                    console.log("webUserId is " + photoObj.webUserId);
                    selectTag = Utils.makePickList({
                        list: obj.webUserList,
                        ...
                        selectedKey: photoObj.webUserId  // key that is to be pre-selected (optional)
                    });

                    // replace text box (for user role) with select tag
                    var emailInputTd = validateObjList["webUserId"].inputBox.parentElement;
                    emailInputTd.innerHTML = "";
                    emailInputTd.appendChild(selectTag);
                }
            } // processRoles
        } // gotRecordById
    ...
    function updateSave() {
        ...
        var myData = getDataFromUI();
        //console.log("myData from getDataFromUI: " + myData);
        ajax("webAPIs/updatePhotoAPI.jsp?jsonData=" + myData, reportUpdate, recordError);
